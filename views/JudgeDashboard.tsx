    ...
    <div className="space-y-10 max-w-6xl mx-auto pb-24">
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
        <div className="space-y-2">
          ...
          <p className="text-slate-400 font-medium tracking-widest uppercase text-xs">Official Score Recording</p>
        </div>
      ...
      <div className="relative group">
        ...
        <input 
          ...
          className="w-full bg-slate-900 border border-white/10 rounded-3xl py-6 pl-16 pr-6 outline-none focus:border-blue-500/30 transition-all text-xl font-medium shadow-2xl"
        />
      </div>
            ...
            <div key={participant.id} className="relative">
              {currentEvent && (
                <ScoreCard 
                  ...
                  initialDeductions={existingScore?.deductions}
                  initialCritique={existingScore?.critique}
                  onSave={(scores, deds, critique) => handleSaveScore(participant.id, scores, deds, critique)}
                />
              )}
              {existingScore && (
                <div className="absolute top-4 right-4 z-10 px-3 py-1 bg-emerald-500/20 border border-emerald-500/30 rounded-full flex items-center gap-1.5 backdrop-blur-md">
                  ...
                  <span className="text-[10px] font-black text-emerald-400 uppercase tracking-widest">Recorded</span>
                </div>
              )}
            </div>
          );
