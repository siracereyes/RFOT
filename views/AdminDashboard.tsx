...
import { Trophy, Users, Shield, Plus, Lock, Unlock, Award, UserPlus, X, Edit3, Check, Layers, GitBranch, Trash2, Key, UserCheck } from 'lucide-react';
import WeightingWizard from '../components/WeightingWizard';
import { Event, EventType, Criterion, Participant, User, UserRole } from '../types';
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ events, participants, users, onAddEvent, onUpdateEvent, onAddParticipant, onAddJudge, onRemoveJudge }) => {
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto">
        ...
        <>
          ...
          {showWizard ? (
            <div className="glass-card p-8 rounded-[2.5rem] border border-white/10 animate-in slide-in-from-bottom-10 duration-500">
              {/* Event Setup Form - No changes to existing logic except Reset */}
                      ...
                      <div className="p-10 text-center space-y-6">
                        ...
                        <div>
                          ...
                          <p className="text-slate-500 mt-2 text-sm leading-relaxed">
                            In Quiz Bee mode, contestants are evaluated per round. Scores are aggregated cumulatively.
                          </p>
                        </div>
                      </div>
                    )}
          ...
          <div className="glass-card rounded-[2.5rem] overflow-hidden border border-white/10 shadow-2xl">
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-white/5">
                {judges.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="px-8 py-20 text-center text-slate-600 italic font-medium">No judge accounts created yet.</td>
                  </tr>
                ) : (
                  judges.map(judge => (
                    <tr key={judge.id} className="hover:bg-white/5 transition-colors group">
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-bold text-lg">{judge.name}</span>
                        </div>
                      </td>
                      <td className="px-8 py-6">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-950/90 backdrop-blur-xl">
          <div className="glass-card w-full max-w-md p-10 rounded-[3rem] border border-white/10 animate-in zoom-in-95 duration-300 shadow-2xl">
            <div className="flex justify-between items-center mb-8">
              <h3 className="text-2xl font-black font-header tracking-tight">Setup Judge Access</h3>
              <button onClick={() => setShowJudgeModal(false)} className="p-2 text-slate-600 hover:text-white transition-colors"><X size={28} /></button>
            </div>
            <div className="space-y-6">
              <div>
                ...
                <input 
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-4 outline-none focus:border-indigo-500 font-bold transition-all" 
                  placeholder="e.g. Judge_Smith"
                />
              </div>
              <div>
                <label className="text-[10px] font-black text-slate-600 uppercase tracking-widest mb-2 block">Password</label>
                <div className="relative">
                  <input 
                    type="text" 
                    value={judgePassword}
                    onChange={(e) => setJudgePassword(e.target.value)}
                    className="w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-4 outline-none focus:border-indigo-500 font-bold transition-all pr-12" 
                    placeholder="Set secure password"
                  />
                  <Key className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-700" size={20} />
                </div>
              </div>
              <div>
                <label className="text-[10px] font-black text-slate-600 uppercase tracking-widest mb-2 block">Assigned Event</label>
                <select 
                  value={assignedEventId}
                  onChange={(e) => setAssignedEventId(e.target.value)}
                  className="w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-4 outline-none focus:border-indigo-500 font-bold transition-all appearance-none"
                >
                  <option value="" className="bg-slate-950 text-slate-500">Select Competition</option>
                  {events.map(e => <option key={e.id} value={e.id} className="bg-slate-950">{e.name}</option>)}
                </select>
              </div>
              <button 
                onClick={handleCreateJudge}
                className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-black uppercase tracking-widest py-5 rounded-2xl shadow-xl shadow-indigo-600/20 active:scale-95 transition-all mt-6"
              >
      ...
      {/* Enroll Participant Modal - existing */}
      {showEnrollModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-950/90 backdrop-blur-xl">
          <div className="glass-card w-full max-w-md p-10 rounded-[3rem] border border-white/10 animate-in zoom-in-95 duration-300 shadow-2xl">
            <div className="flex justify-between items-center mb-8">
